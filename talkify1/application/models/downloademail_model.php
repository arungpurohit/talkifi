...
class Downloademail_model extends CI_Model{
				...
				$newLeadCount = 0;
					...
					if ($this->peeker->message_waiting())
					{
						...
						for($msg_no=1;$msg_no<=$msgCount;$msg_no++)
						{
							...
							if(trim(strpos($message_id,$e->get_message_id()))=="")//check for messageid from remove the duplicity
							{
									$downloadCount++;
									...
									else // means that new client
									{
										...
										$data = array(
											...
											'client_created_by'=>$this->user->id, 
											'client_created_via'=>$channels_id,
											'cmp_unique_id'=>$this->user->cmp_unique_id
										);
