<?php
class repdashboard_model extends CI_Model{
   ...
   public function getTypesVsLeads($type_id){
	...
	public function getChannelssVsLeads($channel_id,$dateFromChannel,$dateToChannel){
				...
				if($dateFromChannel!='' && $dateToChannel!='')
				{
					$dateFromChannel = strtotime($dateFromChannel." 00:00:00"); 
					$dateToChannel = strtotime($dateToChannel." 00:00:00");

					$this->db->where('leads.lead_creation_date >=', $dateFromChannel);
					$this->db->where('leads.lead_creation_date <=', $dateToChannel);
				}
			$rows = $this->db-> get();
